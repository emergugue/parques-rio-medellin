...
function da_get_download_attachments( $post_id = 0, $args = array() ) {
	...
	foreach ( $files as $key => $row ) {
		$files[$key]['attachment_date'] = date_format(new DateTime($files[$key]['attachment_date']), "d/m/Y");
...
function da_display_download_attachments( $post_id = 0, $args = array() ) {
	...
	if ( $count > 0 ) {
		...
		if ( $args['style'] === 'list' ) {
			...
			$html .= '<th class="attachment-title">' . __( 'Archivo', 'download-attachments' ) . '</th>';

			if ( $args['display_caption'] === 1 || ($args['display_description'] === 1 && $args['use_desc_for_title'] === 0) )
				$html .= '<th class="attachment-about">' . __( 'Descripción', 'download-attachments' ) . '</th>';

			if ( $args['display_date'] === 1 )
				$html .= '<th class="attachment-date">' . __( 'Fecha', 'download-attachments' ) . '</th>';

			if ( $args['display_user'] === 1 )
				$html .= '<th class="attachment-user">' . __( 'Autor', 'download-attachments' ) . '</th>';

			if ( $args['display_size'] === 1 )
				$html .= '<th class="attachment-size">' . __( 'Tamaño', 'download-attachments' ) . '</th>';

			if ( $args['display_count'] === 1 )
				$html .= '<th class="attachment-downloads">' . __( 'Descargas', 'download-attachments' ) . '</th>';
		...
		foreach ( $attachments as $attachment ) {
			...
			$name_title = strlen($title) > 35 ? substr($title, 0, 31) . '...' : $title;

			// link
			$html .= '<a href="' . ($options['pretty_urls'] === true ? site_url( '/' . $options['download_link'] . '/' . $attachment['attachment_id'] . '/' ) : plugins_url( 'download-attachments/includes/download.php?id=' . $attachment['attachment_id'] )) . '" class="attachment-link" title="' . $title . '">' . $name_title . '</a>';
			...
			// date
			if ( $args['display_date'] === 1 )
				$html .= '<' . $item_container . ' class="attachment-date">' . ($args['style'] != 'table' ? '<span class="attachment-label">' . __( 'Fecha', 'download-attachments' ) . ':</span> ' : '') . $attachment['attachment_date'] . '</' . $item_container . '> ';

			// user
			if ( $args['display_user'] === 1 )
				$html .= '<' . $item_container . ' class="attachment-user">' . ($args['style'] != 'table' ? '<span class="attachment-label">' . __( 'Autor', 'download-attachments' ) . ':</span> ' : '') . $attachment['attachment_user_name'] . '</' . $item_container . '> ';

			// size
			if ( $args['display_size'] === 1 )
				$html .= '<' . $item_container . ' class="attachment-size">' . ($args['style'] != 'table' ? '<span class="attachment-label">' . __( 'Tamaño', 'download-attachments' ) . ':</span> ' : '') . $attachment['attachment_size'] . '</' . $item_container . '> ';

			// downloads
			if ( $args['display_count'] === 1 )
				$html .= '<' . $item_container . ' class="attachment-downloads">' . ($args['style'] != 'table' ? '<span class="attachment-label">' . __( 'Descargas', 'download-attachments' ) . ':</span> ' : '') . $attachment['attachment_downloads'] . '</' . $item_container . '> ';
