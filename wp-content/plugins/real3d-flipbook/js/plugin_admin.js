...
$(document).ready(function () {
	...
	addOption("general", "pdfPageScale", "text", "PDF page size(between 1 and 2) ",1.5);
	addOption("general", "pdfPageQuality", "text", "PDF page quality(between 0 and 1) ",0.9);
	addOption("general", "pdfPageBgColor", "text", "PDF page background color ","#FFFFFF");
	addOption("general", "pdfUrl", "selectFile", "PDF file url (deprecated) ","");
	...
	addOption("general", "zoomLevels", "text", "Zoom levels","1,1.2,1.5,1.9,2.5,4,6");
	...
	addOption("menu", "btnPrint[enabled]", "checkbox", "Button print",true);
	addOption("menu", "btnPrint[icon]", "text", "Button print CSS class","fa-print");
	addOption("menu", "btnPrint[title]", "text", "Button print title","Print");
		...
		pdf.getPage(creatingPage).then(function getPage(page) {
			...
			setOptionValue("thumbnailHeight", 100*viewport.height/viewport.width);
				...
				renderPdfPage(page, .3, thumbName, function(){
					...
					var pagesCount = pagesContainer.find(".page").length;
					var currentPage = pagesCount + 1;
					var pageItem = createPageHtml("pages["+pagesCount+"]", currentPage,"page "+currentPage, pluginDir+"books/"+bookName+"/page"+currentPage+".jpg", pluginDir+"books/"+bookName+"/thumb"+currentPage+".jpg","");
					pageItem.appendTo(pagesContainer);
					$(".tabs").tabs();
					addListeners();
